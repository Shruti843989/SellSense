  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            ...
            <div className="flex items-center space-x-2.5">
              ...
              <div>
                ...
                <p className="text-sm text-slate-400">
                  Real-time statistical anomaly detection, rolling behavioral drift monitoring, &amp; LLM risk supervision.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              disabled={simulating}
              className="flex items-center space-x-1.5 px-3 py-1.5 rounded-xl bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 text-white text-xs font-bold shadow-lg shadow-red-500/20 transition-all active:scale-95 disabled:opacity-50"
            ...
            <button
              ...
              disabled={simulating}
              className="flex items-center space-x-1.5 px-3 py-1.5 rounded-xl bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 text-white text-xs font-bold shadow-lg shadow-amber-500/20 transition-all active:scale-95 disabled:opacity-50"
              title="Simulate rolling behavioral price drift spike"
            >
              <Activity className="w-3.5 h-3.5" />
              <span>Simulate Price Drift Spike</span>
            </button>
      ...
      <div className={`glass-panel p-5 rounded-2xl border transition-all ${
        isDriftDetected ? 'bg-amber-950/20 border-amber-500/40' : 'bg-slate-900/60 border-slate-800'
      }`}>
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center space-x-2.5">
              <Activity className={`w-5 h-5 ${isDriftDetected ? 'text-amber-400 animate-pulse' : 'text-emerald-400'}`} />
              <span className="text-sm font-extrabold text-white tracking-wide uppercase">System Health &amp; Behavioral Drift Status</span>
          ...
          <div className="flex items-center space-x-4 text-xs">
            <div className="bg-slate-950/60 px-3.5 py-2 rounded-xl border border-slate-800 text-right">
              <span className="text-[10px] text-slate-400 font-bold uppercase block">Recent Avg Price</span>
              <span className="font-mono font-extrabold text-white">₹{metrics.recentAvgPrice ? metrics.recentAvgPrice.toLocaleString() : '1,500'}</span>
            </div>

            <div className="bg-slate-950/60 px-3.5 py-2 rounded-xl border border-slate-800 text-right">
              <span className="text-[10px] text-slate-400 font-bold uppercase block">Price Shift vs Baseline</span>
              <span className={`font-mono font-extrabold ${metrics.priceShiftPct > 35 ? 'text-amber-400' : 'text-slate-300'}`}>
                {metrics.priceShiftPct > 0 ? `+${metrics.priceShiftPct}%` : `${metrics.priceShiftPct}%`}
              </span>
            </div>

            <div className="bg-slate-950/60 px-3.5 py-2 rounded-xl border border-slate-800 text-right">
              <span className="text-[10px] text-slate-400 font-bold uppercase block">Category Concentration</span>
              <span className={`font-mono font-extrabold ${metrics.categoryConcentrationPct > 70 ? 'text-amber-400' : 'text-slate-300'}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="glass-panel p-4 rounded-xl border border-amber-500/20 bg-amber-950/10">
          ...
          <p className="text-[11px] text-slate-400 mt-1">Behavioral drift or anomaly</p>
        </div>
