...
import { MessageSquare, Send, Sparkles, X, Plus, Check, Bot, User, ChevronRight, ChevronLeft } from 'lucide-react';
...
export default function ChatWidget({ isOpen, onClose, onAddToCart }) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden glass-panel flex flex-col h-[540px]">
      ...
      <div className="px-5 py-4 bg-slate-950 border-b border-slate-800 flex items-center justify-between">
        <div className="flex items-center space-x-2.5">
          ...
          <div>
            <h3 className="font-bold text-white text-sm">SellSense Shopping Assistant</h3>
            <p className="text-[11px] text-emerald-400 font-medium">Conversational Intent &amp; Budget Engine</p>
          </div>
        </div>
        ...
        {messages.map((m, idx) => (
          <div key={idx} className={`flex ${m.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[90%] p-3 rounded-2xl ${
              ...
              {/* Horizontal Scrollable Product Carousel in Chat */}
                ...
                <div className="mt-3 pt-2 border-t border-slate-700/50">
                  <div className="flex space-x-2.5 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-slate-700">
                    {m.products.map((prod) => (
                      <div 
                        key={prod.id} 
                        className="w-44 shrink-0 p-2.5 rounded-xl bg-slate-950/90 border border-slate-800 flex flex-col justify-between space-y-2 hover:border-cyan-500/40 transition-all shadow-md"
                      >
                        <img src={prod.image} alt={prod.name} className="w-full h-24 rounded-lg object-cover" />
                        <div>
                          <h5 className="font-bold text-white text-[11px] line-clamp-1">{prod.name}</h5>
                          <p className="text-[10px] text-slate-400 line-clamp-1">{prod.category}</p>
                          <div className="text-emerald-400 font-extrabold text-xs mt-0.5">₹{prod.price?.toLocaleString('en-IN')}</div>
                        </div>

                        <button
                          onClick={() => onAddToCart(prod)}
                          className="w-full py-1.5 rounded-lg bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-bold text-[10px] flex items-center justify-center space-x-1 transition-all active:scale-95"
                        >
        ...
        {loading && (
          <div className="flex justify-start">
            <div className="bg-slate-800 p-3 rounded-2xl text-slate-400 flex items-center space-x-2 text-xs">
              ...
              <span>SellSense Assistant evaluating catalog...</span>
            </div>
          </div>
        )}
      </div>
      ...
      <form onSubmit={handleSend} className="p-3 bg-slate-950 border-t border-slate-800 flex items-center space-x-2">
        <input
          type="text"
          placeholder="Ask for recommendations (e.g. budget is 500)..."
          value={input}
